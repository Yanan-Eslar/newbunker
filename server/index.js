...
// проектная корневая папка на уровень выше server/
...
// Совместимые маршруты (старые клиенты могли запрашивать эти пути)
app.get(["/", "/index.html"], (req, res) => {
  res.sendFile(path.join(PUBLIC_DIR, "index.html"));
});

// Совместимость: старые пути к статическим файлам
app.get("/script.js", (req, res) => res.sendFile(path.join(PUBLIC_DIR, "assets/js/app.js")));
app.get("/data.js", (req, res) => res.sendFile(path.join(PUBLIC_DIR, "assets/js/data.js")));
app.get("/style.css", (req, res) => res.sendFile(path.join(PUBLIC_DIR, "assets/css/style.css")));
app.get("/images/:name", (req, res) => res.sendFile(path.join(PUBLIC_DIR, "images", req.params.name)));

// Сохраняем совместимость для двух маршрутов с картами
app.get("/cards.txt", (req, res) => {
  const filePath = path.join(PUBLIC_DIR, "data", "cards.classic.txt");
  res.setHeader("Cache-Control", "no-store");
  res.setHeader("Content-Type", "text/plain; charset=utf-8");
  if (!fs.existsSync(filePath)) return res.status(404).send("cards.classic.txt not found");
  res.sendFile(filePath);
});

app.get("/cards-fantasy.txt", (req, res) => {
  const filePath = path.join(PUBLIC_DIR, "data", "cards.fantasy.txt");
  res.setHeader("Cache-Control", "no-store");
  res.setHeader("Content-Type", "text/plain; charset=utf-8");
  if (!fs.existsSync(filePath)) return res.status(404).send("cards.fantasy.txt not found");
  res.sendFile(filePath);
});
...
// --- Socket.IO handlers (копия логики сервера, без изменения) ---
io.on("connection", (socket) => {
  ...
  socket.on("reveal-trait", ({ roomCode, playerNumber, traitKey } = {}) => {
    ...
    if (!room || !player || !canReveal(player, Number(playerNumber))) return;
  ...
  socket.on("reveal-all", ({ roomCode, playerNumber, traitKeys } = {}) => {
    const room = getHostRoom(socket, roomCode);
    if (!room) return;
    (Array.isArray(traitKeys) ? traitKeys : []).forEach((traitKey) => setTraitRevealed(room, Number(playerNumber), traitKey));
  ...
  socket.on("ability-request", ({ roomCode, context } = {}, reply) => {
    ...
    const request = { id: createRequestId(), socketId: socket.id, playerName: player.name, actorNumber: player.playerNumber, context, createdAt: Date.now() };
  ...
  socket.on("disconnect", () => {
    rooms.forEach((room) => {
      ...
      if (room.hostId === socket.id) room.gameLog.push("Ведущий отключился"); else room.gameLog.push(`${player.name} отключился`);
      broadcastRoom(room.roomCode);
    });
  });
});

// --- Вспомогательные функции (копия логики) ---
...
function generateRoomCode() { return createRoomCode(); }

function createRequestId() { return `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 8)}`.toUpperCase(); }

function normalizeRoomCode(roomCode) { return String(roomCode || "").trim().toUpperCase(); }

function cleanName(name) { return String(name || "").trim().slice(0, 32); }
...
function getRoomPlayer(room, socketId) { return room?.players?.find((player) => player.socketId === socketId) || null; }

function canReveal(player, playerNumber) { return player.isHost || Number(player.playerNumber) === Number(playerNumber); }

function setTraitRevealed(room, playerNumber, traitKey) {
  if (!traitKey) return;
  if (!room.revealedTraits[playerNumber]) room.revealedTraits[playerNumber] = {};
...
function assignPlayerNumber(room) {
  ...
  for (let number = 1; number <= maxPlayers; number += 1) if (!used.has(number)) return number;
  return null;
}
...
function reconcilePlayerSlots(room) {
  ...
  room.players.forEach((player) => {
    ...
    if (number >= 1 && number <= maxPlayers && !used.has(number)) { player.playerNumber = number; used.add(number); return; }
    player.playerNumber = null;
  });
  room.players.forEach((player) => {
    if (player.playerNumber) return;
    for (let number = 1; number <= maxPlayers; number += 1) {
      if (!used.has(number)) { player.playerNumber = number; used.add(number); return; }
    }
  });
}

function applySharedState(room, state) {
  if (state.generatedPack) room.generatedPack = state.generatedPack;
  if (state.revealedTraits) room.revealedTraits = state.revealedTraits;
  if (Array.isArray(state.excludedPlayers)) room.excludedPlayers = state.excludedPlayers;
  if (state.usedAbilities) room.usedAbilities = state.usedAbilities;
  if (Array.isArray(state.protectedPlayers)) room.protectedPlayers = state.protectedPlayers;
  if (Array.isArray(state.gameLog)) room.gameLog = state.gameLog.slice(-80);
  if (state.resolvedRequestId) room.pendingAbilityRequests = room.pendingAbilityRequests.filter((r) => r.id !== state.resolvedRequestId);
}
...
function sendReply(reply, payload) { try { if (typeof reply === "function") reply(payload); } catch (e) { /* ignore */ } }

function broadcastRoom(roomCode) { const room = rooms.get(roomCode); if (!room) return; io.to(roomCode).emit("room-state", serializeRoom(room)); }
